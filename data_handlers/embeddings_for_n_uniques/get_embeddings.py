...
if __name__ == "__main__":
    ...
    for i in tqdm.tqdm(range(len(to_be_embedded))):
        to_be_embedded[i]["embedding"] = get_embedding(
            to_be_embedded[i]["prompt"], model="text-embedding-ada-002"
        )
    Dataset.from_list(to_be_embedded).push_to_hub(
        "self-critiquing-base-topic-embeddings"
    )