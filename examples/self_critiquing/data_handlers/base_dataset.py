        ...
        for obj in reader:
            for i in range(len(obj["data"]["questions"])):
                ...
                if obj["data"]["passage"]["title"] is not None:
                    ...
                    prompt = f"{obj['data']['passage']['text']}"
                prompt += f"\n\nQuestion: {obj['data']['questions'][i]['question']}\n\nAnswer:"
                data_list.append(
                    {
                        "id": obj["id"],
                        "split": obj["split"],
                        ...
                        "prompt": prompt,
                        "response": f"{obj['data']['questions'][i]['answer']}",
                    }
                )
    return data_list
...
if __name__ == "__main__":
    ...
    test_data = extract_data_continuations(r"test.jsonl\test.jsonl")
    DatasetDict({"test": Dataset.from_list(test_data)}).push_to_hub(
        "self-critiquing-base-test-continuations"
    )
